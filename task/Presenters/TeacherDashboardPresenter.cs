	...
	public class TeacherDashboardPresenter
	{
        ...
        private async void OnDisciplineSelected()
        {
            ...
            try
            {
                ...
                JournalDataSource journalData = await Task.Run(() => _dataAccess.GetJournalDataSource(selectedClass.Id, selectedDiscipline.DisciplineId, currentYear.Id, startDate, endDate));

                DataTable journalTable = BuildJournalTable(journalData);
        ...
        private DataTable BuildJournalTable(JournalDataSource journalData)
        {
            ...
            {
                var colName = $"lesson_{lesson.LessonId}";
                var column = new DataColumn(colName, typeof(string));
            ...
            var gradesDict = journalData.Grades
                .GroupBy(g => (g.StudentId, g.LessonId))
                .ToDictionary(g => g.Key, g => string.Join(", ", g.Select(gr => gr.WorkType == "Н" ? "Н" : gr.Grade?.ToString() ?? "")));
